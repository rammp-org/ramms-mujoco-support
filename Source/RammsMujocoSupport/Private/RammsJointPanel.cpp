...
private:
	...
	EActiveTimerReturnType Drive(double, float Dt)
	{
		...
		if (Pressed.Contains(EKeys::W)) { L += V; R += V; bWheelKey = true; }
		if (Pressed.Contains(EKeys::S)) { L -= V; R -= V; bWheelKey = true; }
		if (Pressed.Contains(EKeys::A)) { L -= V; R += V; bWheelKey = true; }
		if (Pressed.Contains(EKeys::D)) { L += V; R -= V; bWheelKey = true; }
		if (Pressed.Contains(EKeys::SpaceBar)) { L = R = 0.f; bWheelKey = true; }
		...
		auto Rod = [&](const FKey& Up, const FKey& Dn, float& Target, const TCHAR* JL, const TCHAR* JR)
		{
			...
			float Dir = 0.f;
			if (Pressed.Contains(Up)) { Dir -= 1.f; }
			if (Pressed.Contains(Dn)) { Dir += 1.f; }
			if (Dir != 0.f)
			{
...
void ValidateRobot(const TArray<FString>& Args, UWorld* World)
{
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		Report += FString::Printf(TEXT("  rig constraints: %d recorded, %d resolved%s\n"),
			...
			Resolved < Sw->ChaosConstraintComponents.Num() ? TEXT("  << STALE INSTANCE — DELETE AND RE-PLACE") : TEXT(" (OK)"));
