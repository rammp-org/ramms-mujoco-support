...
void URammsMjEndEffectorController::Bind(mjModel* m, mjData* d, const TMap<int32, UMjActuator*>& ActuatorIdMap)
{
	...
	// EVERY name lookup below must be scoped to THIS articulation's attach
	// prefix. Bare suffix matches search the WHOLE compiled scene: with two
	// arms in the model (e.g. the combined mebot_gen3 robot plus this arm as
	// a child actor of the RAMMP base), the first "…pinch"/"…fingers_actuator"
	// belongs to the OTHER robot — the IK then measures error at the other
	// robot's gripper while its Jacobian columns are OUR dofs, i.e. J == 0,
	// dq == 0, and ctrl tracks the sagging qpos every step: the arm slowly
	// falls under gravity and no teleop input has any effect. (Standalone
	// placement worked only because the model had a single match.)
	FString Prefix;
	if (const AMjArticulation* Art = Cast<AMjArticulation>(GetOwner()))
	{
		Prefix = Art->GetPrefixForDiagnostics();
	}
	auto MatchesOurs = [&Prefix](const char* Name, const FString& Suffix) {
		if (!Name)
		{
			return false;
		}
		const FString S(ANSI_TO_TCHAR(Name));
		return S.EndsWith(Suffix) && (Prefix.IsEmpty() || S.StartsWith(Prefix));
	};

	// Find the gripper actuator by name (prefix-scoped, suffix-only fallback
	// for un-prefixed/legacy scenes). It drives a tendon (not a joint), so
	// the base class may omit it from Bindings — a name lookup is robust.
	for (int Pass = 0; Pass < 2 && GripActId < 0; ++Pass)
	{
		for (int a = 0; a < m->nu; ++a)
		{
			const char* nm = mj_id2name(m, mjOBJ_ACTUATOR, a);
			const bool	bMatch = (Pass == 0)
				 ? MatchesOurs(nm, GripperActuatorSuffix)
				 : (nm && FString(ANSI_TO_TCHAR(nm)).EndsWith(GripperActuatorSuffix));
			if (bMatch)
			{
				GripActId = a;
	...
	// Resolve the EE site (prefix-scoped, suffix-only fallback).
	EeSiteId = -1;
	for (int Pass = 0; Pass < 2 && EeSiteId < 0; ++Pass)
	{
		for (int s = 0; s < m->nsite; ++s)
		{
			const char* nm = mj_id2name(m, mjOBJ_SITE, s);
			const bool	bMatch = (Pass == 0)
				 ? MatchesOurs(nm, EndEffectorSiteName)
				 : (nm && FString(ANSI_TO_TCHAR(nm)).EndsWith(EndEffectorSiteName));
			if (bMatch)
			{
				EeSiteId = s;
				break;
	...
	// base_target lookup must ALSO be prefixed — the bare name never matched a
	// compiled (prefixed) body, so the seat-on-mount init and mocap smoothing
	// have been silently disabled everywhere until now.
	if (!BaseTargetBodyName.IsEmpty())
	{
		const FString Prefixed = Prefix + BaseTargetBodyName;
		BaseTargetBodyId = mj_name2id(m, mjOBJ_BODY, TCHAR_TO_ANSI(*Prefixed));
		if (BaseTargetBodyId < 0)
		{
	...
	const char* EeNm = (EeSiteId >= 0) ? mj_id2name(m, mjOBJ_SITE, EeSiteId) : nullptr;
	const char* GripNm = (GripActId >= 0) ? mj_id2name(m, mjOBJ_ACTUATOR, GripActId) : nullptr;
	UE_LOG(LogTemp, Log,
		TEXT("[RammsEE-IK] bound (prefix '%s'): arm actuators=%d gripper=%d '%hs' ee site=%d '%hs' base body=%d base_target=%d mocap=%d track-base=%d"),
		*Prefix, ArmActIds.Num(), GripActId, GripNm ? GripNm : "-", EeSiteId,
		EeNm ? EeNm : "-", BaseBodyId, BaseTargetBodyId, BaseTargetMocapId,
		bTrackBaseFrame ? 1 : 0);
}
...
void URammsMjEndEffectorController::ComputeAndApply(mjModel* m, mjData* d, uint8 /*Source*/)
{
	...
	if (BaseInitStepsLeft > 0 && FreeBaseQposAddr >= 0)
	{
		...
		for (int i = 0; i < 3; ++i)
			d->qpos[FreeBaseQposAddr + i] = tp[i];
		for (int i = 0; i < 4; ++i)
			d->qpos[FreeBaseQposAddr + 3 + i] = tq[i];
		for (int32 k = 0; k < 6; ++k)
...
{
	FScopeLock Lock(&TargetMutex);
	TgtPos[0] = PosMeters.X;
	TgtPos[1] = PosMeters.Y;
	TgtPos[2] = PosMeters.Z;
	bTargetInit = true;
}
